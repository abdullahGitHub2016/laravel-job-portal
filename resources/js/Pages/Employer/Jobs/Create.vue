...
<template>
  <EmployerLayout :title="isEditing ? 'Edit Job Post' : 'Post a New Job'">
    <div class="max-w-3xl mx-auto">
      ...
      <!-- Step Indicator -->
      <div class="flex gap-1.5 mb-6 overflow-x-auto pb-1 flex-nowrap">
        <button
          v-for="(step, i) in steps" :key="i"
          @click="currentStep = i"
          class="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors flex-shrink-0"
          :class="i === currentStep
            ? 'bg-bd-pink-500 text-white'
            : i < currentStep
              ? 'bg-bd-pink-100 text-bd-pink-700'
              : 'bg-slate-100 text-slate-400'"
        >
          <span class="w-5 h-5 rounded-full text-xs flex items-center justify-center font-bold"
            :class="i < currentStep ? 'bg-bd-pink-600 text-white' : ''">
            {{ i < currentStep ? '✓' : i + 1 }}
          </span>
      ...
      <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-8">
        ...
        <div v-show="currentStep === 0" class="space-y-5">
          ...
          <div>
            ...
            <input v-model="form.title" type="text" placeholder="e.g. Senior PHP Developer"
              class="w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:border-bd-pink-400"
        ...
        <div v-show="currentStep === 1" class="space-y-5">
          ...
          <div class="flex items-center gap-3">
            <input type="checkbox" id="is_remote" v-model="form.is_remote" class="accent-bd-pink-500" />
            <label for="is_remote" class="text-sm font-medium text-slate-700">This is a remote job</label>
          </div>

          <div v-if="!form.is_remote" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
          ...
          <div>
            ...
            <div class="flex gap-3 flex-wrap">
              <label v-for="t in ['monthly','yearly','hourly','negotiable']" :key="t"
                ...
                <input type="radio" :value="t" v-model="form.salary_type" class="accent-bd-pink-500" />
                {{ t.charAt(0).toUpperCase() + t.slice(1) }}
              </label>
            </div>
          </div>

          <div v-if="form.salary_type !== 'negotiable'" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
          ...
          <div class="flex items-center gap-3">
            <input type="checkbox" id="show_salary" v-model="form.show_salary" class="accent-bd-pink-500" />
        ...
        <!-- Navigation Buttons -->
        <div class="flex flex-col-reverse sm:flex-row justify-between gap-3 mt-8 pt-6 border-t border-slate-100">
          ...
          <div class="flex gap-2 w-full sm:w-auto">
            ...
            <template v-if="currentStep === steps.length - 1">
              ...
              <button type="button" @click="submitAs('published')" :disabled="form.processing"
                class="px-6 py-2.5 bg-bd-pink-500 hover:bg-bd-pink-600 text-white rounded-lg text-sm font-semibold transition-colors disabled:opacity-60">
            ...
            <button v-else type="button" @click="nextStep"
              class="px-6 py-2.5 bg-bd-pink-500 hover:bg-bd-pink-600 text-white rounded-lg text-sm font-semibold transition-colors">
              Next →
            </button>
          </div>
        </div>
