  ...
  <SeekerLayout title="My Applications">
    <div class="space-y-4">
      <div v-for="app in applications.data" :key="app.id"
        class="bg-white rounded-xl border border-slate-100 shadow-sm p-5 flex items-center gap-4">
      ...
      <div v-if="!applications.data?.length" class="text-center py-20 text-slate-400">
        ...
        <Link :href="route('jobs.index')" class="text-emerald-500 hover:underline text-sm mt-2 inline-block">Browse Jobs →</Link>
      </div>
    </div>
  </SeekerLayout>
</template>
